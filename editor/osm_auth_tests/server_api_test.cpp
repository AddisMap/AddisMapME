...
UNIT_TEST(OSM_ServerAPI_ChangesetAndNode)
{
  ...
  uint64_t changeSetId = api.CreateChangeSet({{"created_by", "AddisMapME Unit Test"},
...

  {
    ...
    api.UpdateChangeSet(changeSetId, {{"created_by", "AddisMapME Unit Test"},
  ...
  changeSetId = api.CreateChangeSet({{"created_by", "AddisMapME Unit Test"},
                                     {"comment", "For test purposes only."}});
  MY_SCOPE_GUARD(guard, changesetCloser);
